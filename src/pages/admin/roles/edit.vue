...
export default defineComponent({
    setup() {
        ...
        //Call API roles
        const getRoles = () => {
            axios.get('http://127.0.0.1:8000/api/roles/edit/' + `${route.params.id}`)
                .then((response) => {
                    formState.role = response.data.name;
                })
                .catch((err) => {
                    console.log(err);
                });
            ...
            Modal.confirm({
                ...
                onOk() {
                    return new Promise((resolve) => {
                        ...
                        .then(() => axios.put('http://127.0.0.1:8000/api/roles/edit/' + `${route.params.id}`, formState)
                            .then((response) => {
