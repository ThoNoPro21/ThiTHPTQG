<template>
    <a-list bordered style="width: 100%;">
        <template #header>
            <div><b>CHỈNH SỬA THỂ LOẠI</b></div>
        </template>
            ...
            <div class="col-12">
                <a-form :model="formState" name="basic"
                    ...
                    <div class="row col-sm-10" style="margin: 10px;">
                        <div class="col-6">
                            <a-form-item label="Tên thể loại" name="theloai"
                                :rules="[{ required: true, message: 'Vui lòng nhập tên thể loại!' }, { type: 'string', min: 2, max: 99, message: 'Tối thiểu 2 kí tự' }, { whitespace: true, message: 'Không được để trống !' }]">
                                <a-input v-model:value="formState.theloai" allow-clear />
                            </a-form-item>
                        </div>
...
export default defineComponent({
    setup() {
        ...
        //Call API thể loại
        const getCats = () => {
            axios.get('http://127.0.0.1:8000/api/theloai/edit/' + `${route.params.id}`)
                .then((response) => {
                    formState.theloai = response.data[0].name_theloai;
        ...
        const getAllCats = () => {
            axios.get('http://127.0.0.1:8000/api/theloai/')
                .then((response) => {
                    category.value = response.data.danhmuc;
                })
                .catch((err) => {
                    console.log(err);
                });
        ...
        const showConfirmSubmit = () => {
            Modal.confirm({
                ...
                .then(() => axios.put('http://127.0.0.1:8000/api/theloai/edit/' + `${route.params.id}`, formState)
                    .then((response) => {
